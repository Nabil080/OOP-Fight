...
function fight($player1, $player2)
{
    ...
    for ($i = $player2->pointDeVie, $x = $player1->pointDeVie; $i > 0 AND $x > 0; $i = $player2->pointDeVie, $x = $player1->pointDeVie) {
        if (in_array('dodge', $player1->skills)) {
            $dodge_p1 = rand(0, 5);
        }
        if (in_array('dodge', $player1->skills)) {
            $dodge_p2 = rand(0, 5);
        }
        if (in_array('crit', $player1->skills)) {
            $crit_p1 = rand(0, 8);
        }
        if (in_array('crit', $player2->skills)) {
            $crit_p2 = rand(0, 8);
        }
        $rand = rand(0, 3);
        ...
        if ($player2->pointDeVie > 0) {
            echo $player2->name . ' a <span class="green">' . $player2->pointDeVie . '</span> hp restants !<br><br>';
        } elseif ($player2->pointDeVie <= 0) {
            echo $player2->name . ' est <span class="text-red-500 font-bold"> MORT </span> !<br><br>';
        }
